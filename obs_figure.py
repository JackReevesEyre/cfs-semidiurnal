"""Plots timeseries of observed air temperature in a single figure.
"""
...
def main(plot_month: int = 1) -> None:
    ...
    for ii in range(len(axts)):
        plot_ts(fig, axts[ii], ts_locs[ii,:], plot_month, obs_locs[ii])
    leg = axts[0].legend(loc='upper left', ncol=1)
        
    # Save figure.
    plotfileformat='png'
    plt.savefig(PLOT_DIR + 'map_with_timeseries_JAN_MAT_OBS' + '.' + plotfileformat,
...
def plot_ts(fig, ax, location, plot_month, obs_loc):
    """ Add time series to axes on pre-made figure.
    """

    # Load obs (already as local time anomaly).
    ds_obs = load_tao(obs_loc)
    ds_obs_loc = order_hour_by_local_time(ds_obs, 'LON')
    
    # Plot lines.
    ax.plot(range(24), ds_obs_loc.TAIR.data,
            label=r'$Obs. \Delta MAT$', color='#1f77b4')
    ax.fill_between(range(24),
                    ds_obs_loc.TAIR.data + ds_obs_loc.TAIR_ER.data,
                    ds_obs_loc.TAIR.data - ds_obs_loc.TAIR_ER.data,
...
def setup_fig_axs():
    ...
    axt4 = fig.add_subplot(gs[1,0])
    axt5 = fig.add_subplot(gs[1,1], sharey=axt4)
    axt6 = fig.add_subplot(gs[1,2], sharey=axt4)
    ...
    return fig, [axt1, axt2, axt3, axt4, axt5, axt6]
...
def load_tao(loc_str):
    ...
    ds = ds.rename({f'TAIR{loc_str.upper()}_DC':'TAIR',
                    f'TAIR{loc_str.upper()}_DCER':'TAIR_ER',
                    'T24HR':'hour'})
